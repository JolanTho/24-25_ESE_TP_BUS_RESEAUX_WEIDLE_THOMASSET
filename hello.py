...
@app.route('/')
def hello_world():
    return 'Hello, World!\n Est-ce que tu peux m acheter une gauffre Jolan ?\n'
...
methods=['GET', 'POST']
@app.route('/api/request/', methods=['GET', 'POST', 'PUT', 'PATCH', 'DELETE'])
@app.route('/api/request/<path>', methods=['GET', 'POST', 'PUT', 'PATCH', 'DELETE'])
def api_request(path=None):
    ...
    if request.method == 'GET':
        return 'Vous avez réussi avec rested :) \n'
        
    if request.method == 'PATCH':
        data = request.get_json()
        resp["PATCH"] = {
            "updated_data": data,
            "message": f"Resource at '{path}' has been partially updated.",
        }
        return jsonify(resp), 200  # Code de statut 200 (OK)

    
    if request.method == 'DELETE':
        resp["DELETE"] = {
            "message": f"Resource at '{path}' has been deleted.",
        }
